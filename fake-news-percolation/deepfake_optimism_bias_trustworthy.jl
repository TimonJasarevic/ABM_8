# deepfake_optimism_bias.jl
# Non-Sobol experiment to show emergent population-level optimism bias.
using Graphs
...
Base.@kwdef struct SimConfig
    ...
    loss_aversion::Float64 = 1.0  # prospect-theory loss-aversion coeff λ_LA (linear, α=β=1); swept via
                                  # the design (col 7). Scales the reputational LOSS only in
...
function run_optimism_sweep(; design_path::String="sobol/optimism_design.csv", num_reps::Int=20, write_full::Bool=false)
    timestamp = Dates.format(now(), "yyyy_mm_dd_HHMM")
    out_dir = joinpath("data", "sweep_$timestamp")
    mkpath(out_dir)

    # Design CSV columns, in order:
    ...
    open(joinpath(out_dir, "experiment_info.txt"), "w") do io
        println(io, "Deepfake Percolation ABM — Optimism-bias quick sweep")
        println(io, "================================================")
        println(io, "Date: ", now())
        println(io, "Julia Threads: ", Threads.nthreads())
        println(io, "Design source: ", design_path, " (fixed one-factor design)")
        println(io, "Design points: ", NUM_POINTS)
    ...
    println("Loaded $NUM_POINTS design points from $design_path.")
    ...
    Threads.@threads for idx in 1:TOTAL_SIMS
        ...
        # Emergent optimism-bias metrics.
        # true_prob is the actual probability that a message is true.
        # mean_true_belief is the average final belief across agents.
        # belief_error > 0 means agents are over-optimistic.
        mean_true_belief = mean([b.alpha / (b.alpha + b.beta) for b in sim.net.true_belief])
        true_prob = 1.0 - prev
        belief_error = mean_true_belief - true_prob
        ...
        sim_dfs[idx] = DataFrame(
            ...
            mean_true_belief = mean_true_belief,
...
function write_optimism_design(path::String="sobol/optimism_design.csv")
    # One-factor design: vary fake-news prevalence only.
    # Uses 21 values, from 0.00 to 1.00 in steps of 0.05.
    # log10_lambda = 0.39794 corresponds to rationality ≈ 2.5.
    mkpath(dirname(path))
    p_fake_values = collect(0.0:0.05:1.0)
    open(path, "w") do io
        println(io, "v_cost,loss,tpr,fpr,p_fake,log10_lambda,loss_aversion")
        for p_fake in p_fake_values
            println(io, "0.2,2.0,0.90,0.10,$(round(p_fake, digits=2)),0.39794,1.0")
        end
    end
    println("Wrote $(length(p_fake_values)) p_fake values to $path")
    return path
end

if !@isdefined(_INCLUDED_AS_MODULE)
    design_path = write_optimism_design()
    run_optimism_sweep(design_path=design_path, num_reps=20, write_full=false)
end
