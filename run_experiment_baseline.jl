# run_experiment_baseline.jl, the uniform-random (non-influencer) BASELINE variant.
# Identical to run_experiment_influencers.jl except the cascade SEED POOL: each cascade
# seeds from a uniform-random agent (all_nodes) instead of the top-degree hubs
# (find_influencers). This is the non-influencer configuration, paired 1:1 by global_sim_id
# with the influencer sweep (same sobol/design.csv, same thread_seed) for the seeding comparison.
using Graphs
...
function run_sobol_sweep(; design_path::String="sobol/design.csv", num_reps::Int=30, write_full::Bool=false, out_tag::String="")
    ...
    open(joinpath(out_dir, "experiment_info.txt"), "w") do io
        ...
        println(io, "Seeding: uniform-random over all agents (baseline / non-influencer)")
    ...
    Threads.@threads for idx in 1:TOTAL_SIMS
        ...
        # sen_welfare per simulation from the agents' normalized payoffs (matches Python utils.sen_welfare)
