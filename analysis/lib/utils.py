...
def gini_negatives(x):
    ...
    The standard Gini numerator is normalised by ``mean(|x|)`` instead of ``mean(x)``, which
    keeps the index in [0, 1] for signed data. The 'polarised' scenario assigns the whole
    negative total to one unit and the whole positive total to another. Reduces to the classical
...
def cohens_d_av(group1, group2):
    """d_av for paired/two-condition data (convention due to Cumming 2012; popularised by Lakens
    2013): (mean(g1) - mean(g2)) divided by the AVERAGE of the two SDs. Unlike d_z it does NOT
    shrink with the inter-condition correlation, so it is comparable across designs. It is the
    appropriate statistic under common-random-number pairing, where d_z = d_av / sqrt(2(1-r))
    (exact under equal condition variances) exceeds d_av for r > 0.5.
    """
...
def bootstrap_ci(data, statistic=np.mean, confidence=0.95, n_resamples=9999,
                 method="BCa", seed=0, batch=None):
    """Bootstrap confidence interval via ``scipy.stats.bootstrap``: BCa (Kirby & Gerlanc 2013)
    for n <= 5000, percentile above (see the fallback note below). No normality assumption
    is made, unlike a +/-1.96*SE interval. ``data`` is a 1-D sample (for a paired mean
    difference, pass the difference array with ``statistic=np.mean``).
    """
    ...
    if method == "BCa" and data.size > 5000:
        method = "percentile"
    # ``batch`` (scipy default None) chunks the resampling to bound peak memory without changing
    # the interval; pass e.g. batch=100 for the ~491k-row reduce that would otherwise allocate a
    # (n_resamples, n) matrix (~37 GiB).
    kw = dict(n_resamples=n_resamples, confidence_level=confidence, method=method, batch=batch)
    try:
...
def block_bootstrap_ci(a, b, blocks, stat="d_av", confidence=0.95, n_resamples=9999, seed=0):
    """Cluster (block) bootstrap percentile CI for a paired two-condition statistic, resampling
    the unique blocks with replacement (Cameron & Miller 2015). The block, not the simulation,
    is the exchangeable unit of the Saltelli sweep. Supported statistics: ``"d_av"`` =
    (mean(b) - mean(a)) / ((sd(a) + sd(b))/2), matching ``cohens_d_av(b, a)``; ``"mean_diff"`` =
    mean(b - a). Both are functions of first and second moments, so each resample is evaluated
    exactly from per-block sufficient statistics (n, sum, sum of squares) at O(n_resamples * G)
    cost instead of O(n_resamples * n). Percentile interval only (BCa's jackknife acceleration
    is not defined blockwise).
    """
    a = np.asarray(a, dtype=float)
    b = np.asarray(b, dtype=float)
    blocks = np.asarray(blocks)
    m = np.isfinite(a) & np.isfinite(b)
    a, b, blocks = a[m], b[m], blocks[m]
    if a.size < 3:
        return (float("nan"), float("nan"))
    _, inv = np.unique(blocks, return_inverse=True)
    G = int(inv.max()) + 1
    n_g = np.bincount(inv, minlength=G).astype(np.float64)
    sa = np.bincount(inv, weights=a, minlength=G)
    sa2 = np.bincount(inv, weights=a * a, minlength=G)
    sb = np.bincount(inv, weights=b, minlength=G)
    sb2 = np.bincount(inv, weights=b * b, minlength=G)
    rng = np.random.default_rng(seed)
    idx = rng.integers(0, G, size=(n_resamples, G))
    n = n_g[idx].sum(axis=1)
    ma = sa[idx].sum(axis=1) / n
    mb = sb[idx].sum(axis=1) / n
    if stat == "mean_diff":
        vals = mb - ma
    elif stat == "d_av":
        va = (sa2[idx].sum(axis=1) - n * ma ** 2) / (n - 1)
        vb = (sb2[idx].sum(axis=1) - n * mb ** 2) / (n - 1)
        s = (np.sqrt(np.maximum(va, 0.0)) + np.sqrt(np.maximum(vb, 0.0))) / 2.0
        with np.errstate(invalid="ignore", divide="ignore"):
            vals = np.where(s > 0, (mb - ma) / s, np.nan)
    else:
        raise ValueError(f"unknown stat {stat!r}")
    vals = vals[np.isfinite(vals)]
    if vals.size == 0:
        return (float("nan"), float("nan"))
    lo = np.percentile(vals, 100.0 * (1.0 - confidence) / 2.0)
    hi = np.percentile(vals, 100.0 * (1.0 + confidence) / 2.0)
...
def tost_clustered(diff, clusters, sesoi=0.05, alpha=0.05, expect_sizes=None):
    """Cluster-robust (CR1) Two One-Sided Tests of equivalence on a mean paired difference,
    clustering on ``clusters`` (df = G - 1). Equivalence band is +/- ``sesoi`` in raw metric
    units. With balanced clusters this equals the one-sample t on the G cluster means
    (u_g = m*(mean_g - md) makes the two variances algebraically identical); with unbalanced
    cells CR1 keeps the design-point-weighted point estimate. Pass ``expect_sizes`` (a set of
    admissible cluster sizes) to assert the cluster structure, or leave None to skip the check.
    The block-cell-mean t is returned alongside as a sensitivity. This is the reach-equivalence
    statistic of the seeding comparison; the unclustered case is :func:`tost_paired`.
    """
    from scipy import stats as sps
    import pandas as pd
    ...
    n = diff.size
    md = float(diff.mean())
    g = pd.Series(diff - md).groupby(clusters)
    u = g.sum().values
    G = u.size
    assert G >= 2
    sizes = g.size().values
    if expect_sizes is not None:
        assert set(np.unique(sizes)).issubset(expect_sizes), np.unique(sizes)
    se = float(np.sqrt(G / (G - 1.0) * (u ** 2).sum()) / n)
    dfree = G - 1
    p_lo = float(sps.t.sf((md + sesoi) / se, dfree))   # H0: diff <= -sesoi
    p_hi = float(sps.t.cdf((md - sesoi) / se, dfree))  # H0: diff >= +sesoi
    t95 = float(sps.t.ppf(0.95, dfree))
    t975 = float(sps.t.ppf(0.975, dfree))
    cm = pd.Series(diff).groupby(clusters).mean().values
    se_cm = float(cm.std(ddof=1) / np.sqrt(G))
    return {
        "n_design_points": int(n),
        "n_blocks": int(G),
        "cluster_sizes": sorted(int(s) for s in np.unique(sizes)),
        "mean_diff": md,
        "se_cr1": se,
        "df": int(dfree),
        "ci90": [md - t95 * se, md + t95 * se],
        "ci95": [md - t975 * se, md + t975 * se],
        "p_lower": p_lo,
        "p_upper": p_hi,
        "p_tost": max(p_lo, p_hi),
        "equivalent": bool(max(p_lo, p_hi) < alpha),
        "smallest_passing_sesoi": float(abs(md) + t95 * se),
        "p_zero_two_sided": float(2.0 * sps.t.sf(abs(md) / se, dfree)),
        "cellmeans_sensitivity": {
            "mean_diff": float(cm.mean()),
            "se": se_cm,
            "ci90": [float(cm.mean()) - t95 * se_cm, float(cm.mean()) + t95 * se_cm],
        },
    }


def pooled_sd(a, b):
    """Pooled (within-group) standard deviation for two independent samples,
    ``sqrt(((n_a-1) Var(a) + (n_b-1) Var(b)) / (n_a + n_b - 2))``."""
    a = np.asarray(a, dtype=float)
    b = np.asarray(b, dtype=float)
    return float(np.sqrt(((a.var(ddof=1) * (len(a) - 1)) + (b.var(ddof=1) * (len(b) - 1)))
                         / (len(a) + len(b) - 2)))


def cohens_d_pooled(a, b):
    """Cohen's d_s for two independent groups: ``(mean(a) - mean(b)) / pooled_sd(a, b)``
    (NaN if the pooled SD is zero). The independent-samples counterpart of :func:`cohens_d_av`."""
    a = np.asarray(a, dtype=float)
    b = np.asarray(b, dtype=float)
    sp = pooled_sd(a, b)
